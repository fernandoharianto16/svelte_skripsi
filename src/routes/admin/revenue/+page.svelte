<script>
    ...
    async function loadTransactions() {
        try {
            ...
            const res = await api.get(`/transaction/admin`);
    ...
    $: grossEarnings = completedTx.reduce(
        (sum, tx) => sum + tx.total_payment,
        0,
    );
...
<div class="revenue-page">
    <div class="header">
        <h2>Pendapatan Admin</h2>
    </div>

    <div class="earnings-container">
        <div class="card card-net">
            <!-- Diubah agar lebih jelas bahwa ini total biaya admin yang dikumpulkan -->
            <span class="card-label">Total Pendapatan Admin</span>
            <span class="card-value"
                >Rp {totalAdminFee.toLocaleString("id-ID")}</span
            >
        </div>
    </div>
            ...
            <tbody>
                {#each filteredTransactions as tx}
                    <tr>
                        ...
                        <!-- Ditampilkan sebagai uang yang keluar/diteruskan ke penjual -->
                        <td class="text-danger"
                            >- Rp {tx.seller_income.toLocaleString("id-ID")}</td
                        >

                        <!-- Bagi Admin, admin_fee adalah keuntungan bersih Anda -->
                        <td class="text-success font-weight-bold"
                            >Rp {tx.admin_fee.toLocaleString("id-ID")}</td
                        >
