<script>
  ...
  async function loadNegotiations() {
    try {
      const seller_id = getAuth().currentUser.uid;
      // loading = true;
      // Cukup panggil tanpa melempar ID di parameter, karena backend membaca token JWT Anda
      const res = await api.get(`/seller/negotiations`, seller_id);
      // console.log(res);
  ...
  async function acceptNego(negoId) {
    ...
    try {
      Swal.showLoading();
      await api.patch(`/seller/negotiations/${negoId}/accept`);
      const seller_id = getAuth().currentUser.uid;
      ...
      negotiations = negotiations.map((n) =>
        n.id === negoId
          ? {
              ...n,
              status: "rejected",
              counter_count: (n.counter_count || 0) + 1,
            }
          : n,
      );
      Swal.showLoading();
      await api.patch(`seller/negotiations/${negoId}/reject`, seller_id);
...
{:else}
  <table border="1" cellpadding="8">
        ...
        <tr>
          ...
          <td class="action-cell">
            <div class="action-wrapper">
              {#if nego.status === "pending" && (nego.counter_count || 0) < 3}
      <button
    ...
    {:else if nego.status === "rejected" && (nego.counter_count || 0) >= 3}
      <span style="color: #ef4444; font-size: 0.8rem; font-weight: bold;">
        Selesai
      </span>
  ...
  <div class="modal-backdrop" on:click={closeNegoModal}>
    <div class="modal-box" on:click|stopPropagation>
      <div class="modal-header-detail">
        ...
        <button class="btn-close-detail" on:click={closeNegoModal}>&times;</button>
      </div>
          ...
          <div class="timeline">
            {#each negoHistory.sort((a, b) => new Date(b.created_at) - new Date(a.created_at)) as step}
              ...
              >
                <div class="card-meta">
                  <span class="sender"
                    >Oleh: {step.origin_type ? step.origin_type.toUpperCase() : ''}</span
                  >
                  <span class="time"
                    >{step.created_at ? new Date(step.created_at).toLocaleTimeString("id-ID") : ''}</span
                  >
                </div>
                <div class="card-price">
                  Nominal: <strong>Rp {step.offered_price ? step.offered_price.toLocaleString("id-ID") : 0}</strong>
                </div>
                <div class="card-status">
                  Counter Hit: <span class="badge-mini">{step.counter_count || 0}</span>
                  | Status Lembar:
          ...
          {#if negoMain?.status === "pending" && negoMain?.origin_type === "seller"}
            <div class="modal-action-footer">
              ...
              <button
                ...
                on:click={() => rejectNego(negoMain.id)}
                >Tolak</button
              >
            </div>
          {/if}
